...
Graph::Graph(std::vector<double>& data, double xscale) : 
	...
	_data(data),
	_graph_size_y(GRAPH_SIZE_Y)
{
	double max_val = *std::max_element(data.begin(), data.end());

	while(max_val > _graph_size_y)
	{
		_graph_size_y *= 1.5;
	}
	_map = std::vector<std::vector<u_short>>(GRAPH_SIZE_X+1,std::vector<u_short>(_graph_size_y+1,0));
	_num_data = _data.size();
	_scale_x = xscale*_graph_size_y/(max_val+1);
	for (u_int i = 0; i < _num_data; ++i)
	{
		u_int xpos = (i+1)*(GRAPH_SIZE_X/(_num_data+1));
		u_int ypos = _data[i]*_graph_size_y/(max_val+1);
		_map[xpos][ypos] = 1;
