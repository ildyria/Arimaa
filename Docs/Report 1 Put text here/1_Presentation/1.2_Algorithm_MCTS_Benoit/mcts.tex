...
The \textit{MAX} node at the top still has two other children nodes that we need to generate and search.
...
What this tells us is that we should take the move that leads to the middle  \textit{MIN} node, since it will lead to the best possible state for us one full move down the road.
...
\centering
	\begin{minipage}[b]{0.45\linewidth}
		...
		\caption{\label{fig:6}Create a leaf at depth 2 with parent the second leaf at depth 1 and run the simulation (\textit{loss}), update the odds value of the node and making it less interesting than the fourth node. Therefore the algorithm will now work on the fourth node.}
	\end{minipage}%
	\hspace*{1cm}
	\begin{minipage}[b]{0.45\linewidth}
		\centering
