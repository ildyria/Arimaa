...
namespace api_v2
{
	...
	void Game::setLastMove(u_long lm)
	{
		_lastMove = lm;	
	}

	void Game::setState(std::vector<u_long> state)
	{
		_board->import(state);
	}

	std::vector<u_long> Game::getState()
	{
	...
	void Game::displayASCII()
	{
		_game->diplay_board(_board);
	}
