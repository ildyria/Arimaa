...
//		}
	...
	void Mcts::cleanTree_v2(std::vector<Node> &T, Node* NewRoot)
	{
		auto ListOfNodes = T[0]->getChildren();
		Node* ptTemp = ListOfNodes.first;
		for (int i = 0; i < ListOfNodes.second; ++i)
		{
			if(ptTemp != NewRoot) ptTemp.clearParent(); // define all first children but the chosen one as useless !
			ptTemp++;
		}
	...
	std::pair<Node*, u_int> Mcts::findNextHole(Node* startwith)
	{
		Node* start = startwith;
		Node* end = _tree[_tree.size() - 1];
		...
		u_int = 0;
		Node* tmp = start;
		while(tmp != end && !tmp.hasParent())
		{
			i++;
			tmp++;
		}

		return std::pair<Node*, u_int>(start,i);
	}

	void Mcts::copySubTree(std::pair<Node*, u_int>& from, std::pair<Node*, u_int>& to)
	{
		Node* node_from = from.first;
		Node* node_to = from.first;
		for (u_int i = 0; i < from.second; ++i)
		{
			*node_to = *node_from;
